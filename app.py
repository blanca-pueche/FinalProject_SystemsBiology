...
# Custom CSS to change background and text
st.markdown(
    """
    <style>
    body {
...
if mesh_id:
    ...
    if disease:
        ...
        if uploaded_file:
            ...
            # Convert the Ensembl gene IDs into clickable HTML links
            ...
            # Use st.dataframe with the HTML rendered as a scrollable table
            ...
            st.download_button("📥 Download Pathways CSV", df_selected_with_links.to_csv(index=False), "genes.csv", "text/csv")
            ...
            if top_pathways is not None:
                ...
                # Filtra los genes importantes del pathway seleccionado
            ...
            if not drug_df.empty:
                ...
                # Apply links to the drugs column
                drug_df_with_links = drug_with_links(drug_df)
    
                # Display the drug interactions with clickable drug links
